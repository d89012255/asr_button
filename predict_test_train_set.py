...
def add_silence_to_wav_file(input_file, output_file, silence_duration_sec):
    ...
    silence_data2 =  np.zeros(2*silence_duration_frames, dtype=np.int16)   

    # 將空白音訊資料加到音訊的前後
    new_audio_data = np.concatenate((silence_data, audio_data, silence_data2))
...
def post_process_3(input):
    ...
    first_record = ["i","yi","i"]
    mix = [second_record,third_record,forth_record,fifth_record,sixth_record, ninth_record,seventh_record,eighth_record,first_record]
    table= ["第二筆","第三筆","第四筆","第五筆","第六筆","第九筆","第七筆","第八筆","第一筆"]
    all = ""
    for i in range(len(input)):
        all+=(input[i][:-1]+" ")
    temp_for_check = all
...
for i in range(1):                                         # numbers from 0 to 9     
    sound = AudioSegment.from_wav(f'temp88.wav')
    #get 分貝數
    dBFS = sound.dBFS
    print(dBFS)
